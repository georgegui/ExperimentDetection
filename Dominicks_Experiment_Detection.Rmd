...
package_list <- c('mixtools', 'gridExtra', 'lfe', 'matrixStats', 
                  ...
                  'data.table', 'mixtools', 'scales', 'stargazer')
...
GetNpMix <- function(
  ...
  unique_products <- dt[, sum(formatted_price != 0), by = upc][order(-V1), upc]
  # cap the product limit to save time
  if(length(unique_products) > n_product_limit){
    unique_products <- unique_products[1:n_product_limit] 
    dt <- dt[upc %in% unique_products]
  } 
